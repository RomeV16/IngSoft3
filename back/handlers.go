...
// ── módulo de reportes avanzados (sin tests — cobertura nueva ~0%) ─────────────
...
func (a *API) handleDepartmentSummary(w http.ResponseWriter, r *http.Request) {
	...
	summaries, err := a.store.DepartmentSummaries(from, to)
	if err != nil {
...
func (a *API) handleExportPayroll(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	rows, err := a.store.PayrollByPeriod(period)
	if err != nil {
		http.Error(w, internalErrorMsg, http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", "text/csv")
	w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=payroll-%s.csv", period))
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, "id,employee_id,period,base_salary,net\n")
	for _, row := range rows {
		fmt.Fprintf(w, "%d,%d,%s,%.2f,%.2f\n",
			row.ID, row.EmployeeID, row.Period, row.BaseSalary, row.NetPay)
	}
}
...
func buildReportFilter(r *http.Request) reportFilter {
	q := r.URL.Query()
	f := reportFilter{}
	...
	f.IncludeZero = q.Get("include_zero") == "true"
	return f
}
